...
/* ============================================================ */
/* HINTERGRUND-DESIGN (Themes + Schieberegler, in localStorage) */
/* ============================================================ */

// Standard entspricht dem bisherigen Grün (#1B5E20)
const BG_DEFAULT = { h: 130, s: 60, l: 19 };
let   bgSettings = { ...BG_DEFAULT };

// Vorgefertigte Themes (Farbton/Sättigung/Helligkeit)
const BG_THEMES = [
  { name: 'Grün',      h: 130, s: 60, l: 19 },
  { name: 'Blau',      h: 215, s: 55, l: 18 },
  { name: 'Türkis',    h: 182, s: 48, l: 18 },
  { name: 'Anthrazit', h: 220, s: 12, l: 15 },
  { name: 'Bordeaux',  h: 349, s: 45, l: 16 },
  { name: 'Violett',   h: 265, s: 35, l: 18 },
];

/** Hintergrund-Variablen auf :root setzen (wird vom body-Verlauf genutzt). */
function applyBackground() {
  const r = document.documentElement;
  r.style.setProperty('--bg-h', bgSettings.h);
  r.style.setProperty('--bg-s', bgSettings.s + '%');
  r.style.setProperty('--bg-l', bgSettings.l + '%');
}

/** Aktuelle Einstellung in localStorage sichern (wie Sprache/Geschwindigkeit). */
function saveDesignSettings() {
  try { localStorage.setItem('skipbo-bg', JSON.stringify(bgSettings)); }
  catch (e) { console.warn('⚠️ LocalStorage nicht verfügbar (Design)'); }
}

/** Gespeicherte Einstellung laden und anwenden (beim Start). */
function loadDesignSettings() {
  try {
    const saved = JSON.parse(localStorage.getItem('skipbo-bg'));
    if (saved && typeof saved.h === 'number'
              && typeof saved.s === 'number'
              && typeof saved.l === 'number') {
      bgSettings = { h: saved.h, s: saved.s, l: saved.l };
    }
  } catch (e) { /* Standard behalten */ }
  applyBackground();
}

/** Regler/Theme-Markierung an den aktuellen Stand angleichen. */
function syncDesignControls(updateSliders = true) {
  const ov = document.getElementById('design-overlay');
  if (!ov) return;
  const hR = ov.querySelector('#design-hrange');
  const lR = ov.querySelector('#design-lrange');
  if (updateSliders) { hR.value = bgSettings.h; lR.value = bgSettings.l; }
  ov.querySelector('#design-hval').textContent = bgSettings.h + '°';
  ov.querySelector('#design-lval').textContent = bgSettings.l + '%';
  ov.querySelectorAll('.design-theme').forEach((el, i) => {
    const th = BG_THEMES[i];
    el.classList.toggle('active',
      th.h === bgSettings.h && th.s === bgSettings.s && th.l === bgSettings.l);
  });
}

/** Design-Overlay öffnen (einmalig aufbauen, dann nur einblenden). */
function openDesignOverlay() {
  let ov = document.getElementById('design-overlay');
  if (!ov) {
    ov = document.createElement('div');
    ov.id = 'design-overlay';
    ov.className = 'design-overlay';
    ov.innerHTML = `
      <div class="design-panel">
        <div class="design-handle"></div>
        <h2>🎨 ${t('designTitle')}</h2>
        <div class="design-sec">${t('designThemes')}</div>
        <div class="design-themes" id="design-themes"></div>
        <div class="design-sec">${t('designFine')}</div>
        <div class="design-ctrl">
          <label><span>${t('designBright')}</span><span id="design-lval"></span></label>
          <input type="range" id="design-lrange" min="8" max="32" step="1">
        </div>
        <div class="design-ctrl">
          <label><span>${t('designHue')}</span><span id="design-hval"></span></label>
          <input type="range" id="design-hrange" min="0" max="360" step="1">
        </div>
        <button class="design-close" id="design-close">${t('designDone')}</button>
      </div>`;
    document.body.appendChild(ov);

    // Theme-Buttons aufbauen
    const grid = ov.querySelector('#design-themes');
    BG_THEMES.forEach((th) => {
      const d = document.createElement('div');
      d.className = 'design-theme';
      d.innerHTML =
        `<div class="design-sw" style="background:radial-gradient(ellipse at 50% 30%,`
        + `hsl(${th.h},${th.s}%,${th.l + 8}%),hsl(${th.h},${th.s}%,${th.l}%))"></div>${th.name}`;
      d.addEventListener('click', () => {
        bgSettings = { h: th.h, s: th.s, l: th.l };
        applyBackground(); saveDesignSettings(); syncDesignControls(true);
      });
      grid.appendChild(d);
    });
    ...
    const lR = ov.querySelector('#design-lrange');
    const onSlide = () => {
      bgSettings.h = parseInt(hR.value);
      bgSettings.l = parseInt(lR.value);
      applyBackground(); saveDesignSettings(); syncDesignControls(false);
    };
    hR.addEventListener('input', onSlide);
    lR.addEventListener('input', onSlide);

    ov.querySelector('#design-close')
      .addEventListener('click', () => ov.classList.remove('open'));
  }
  syncDesignControls(true);
  ov.classList.add('open');
}

/** Schnellmenü-Eintrag „🎨 Design". */
function menuDesign() {
  closeQuickMenu();
  openDesignOverlay();
}

// Beim Laden gespeicherte Einstellung anwenden (analog addStockHighlightStyles)
loadDesignSettings();
